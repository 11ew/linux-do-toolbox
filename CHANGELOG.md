...
## 文件结构
- `Linux.do 工具箱.user.js` — 主脚本（当前 v3.10.0）
...
### v3.10.0 — 新增 Sunset / Forest 主题
**用户需求：** 工具箱新增主题，两个，你自己选主题

**实现：**
- **新增 `sunset` 主题**：暖橙→蜜桃→粉霞渐变背景，搭配橙粉强调色，整体更偏日落氛围
- **新增 `forest` 主题**：深绿夜林渐变背景，搭配薄荷绿和黄绿强调色，整体更沉稳耐看
- **主题切换扩展**：主题循环从 11 个扩展到 13 个，顺序更新为 dark→light→ocean→pink→red→white→gradient→particle→pinkwhite→royalgold→streamline→sunset→forest
- **主题图标补充**：`sunset` 使用 `🌅`，`forest` 使用 `🌲`
- **文档与元信息同步**：脚本描述、README 主题数量、版本号统一更新到 `v3.10.0`
